...
.li-meta{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}
.li-tag{padding:4px 8px;background:rgba(30,58,79,0.6);border-radius:4px;font-size:10px;color:#cbd5e1;border:1px solid rgba(30,58,79,0.8)}
.li-tag.quality{background:linear-gradient(135deg,rgba(20,184,166,0.25),rgba(20,184,166,0.15));color:#7ee8c9;border-color:rgba(20,184,166,0.4)}
.li-tag.resolution{background:linear-gradient(135deg,rgba(168,85,247,0.25),rgba(168,85,247,0.15));color:#d8b4fe;border-color:rgba(168,85,247,0.4)}
.li-tag.language{background:linear-gradient(135deg,rgba(59,130,246,0.25),rgba(59,130,246,0.15));color:#bfdbfe;border-color:rgba(59,130,246,0.4)}
.li-tag.video_codec{background:linear-gradient(135deg,rgba(236,72,153,0.25),rgba(236,72,153,0.15));color:#f9a8d4;border-color:rgba(236,72,153,0.4)}
.li-tag.audio_codec{background:linear-gradient(135deg,rgba(251,146,60,0.25),rgba(251,146,60,0.15));color:#fdba74;border-color:rgba(251,146,60,0.4)}
.li-tag.source_type{background:linear-gradient(135deg,rgba(239,68,68,0.25),rgba(239,68,68,0.15));color:#fca5a5;border-color:rgba(239,68,68,0.4)}
.li-tag.file_size{background:linear-gradient(135deg,rgba(34,197,94,0.25),rgba(34,197,94,0.15));color:#86efac;border-color:rgba(34,197,94,0.4)}
.li-bottom{padding:12px 16px;background:rgba(30,58,79,0.2)}
.li-btn{padding:12px 20px;background:linear-gradient(135deg,#14B8A6,#0d9488);color:#0c1520;border:none;border-radius:8px;cursor:pointer;font-weight:700;font-size:14px;width:100%;transition:all 0.2s}
.li-btn:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(20,184,166,0.3)}
...
.ft a{color:#14B8A6;text-decoration:none}
...
.ext-filters{display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap}
.ext-select{padding:8px 12px;background:rgba(30,58,79,0.6);border:1px solid rgba(102,126,234,0.3);border-radius:8px;color:#e5e7eb;font-size:13px;cursor:pointer}
.ext-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:12px}
.ext-card{background:rgba(22,34,48,0.8);border:1px solid rgba(30,58,79,0.6);border-radius:12px;overflow:hidden;cursor:pointer;transition:all 0.2s}
.ext-card:hover{border-color:rgba(102,126,234,0.5);box-shadow:0 4px 12px rgba(102,126,234,0.2)}
.ext-card-body{padding:16px;display:flex;flex-direction:column;gap:10px}
.ext-provider{font-weight:600;color:#fff;font-size:14px}
.ext-quality{display:inline-block;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;padding:4px 12px;border-radius:6px;font-size:11px;font-weight:700;width:fit-content}
.ext-info{font-size:12px;color:#cbd5e1}
.ext-host{padding:8px;background:rgba(30,58,79,0.4);border-radius:6px;font-size:11px;color:#a5b4fc}
.ext-stats{display:flex;gap:8px;margin-top:4px}
.ext-stat{display:flex;flex-direction:column;gap:2px;font-size:11px}
.ext-stat-label{color:#6b7280}
.ext-stat-value{color:#fff;font-weight:600}
.ext-btn{padding:8px 16px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:700;font-size:12px;width:100%;transition:all 0.2s}
.ext-btn:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(102,126,234,0.3)}
.ext-details{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.8);z-index:1000;align-items:center;justify-content:center;padding:16px}
.ext-details.show{display:flex}
.ext-details-inner{background:linear-gradient(135deg,#0a1928,#111827);border:1px solid rgba(102,126,234,0.3);border-radius:16px;max-width:500px;width:100%;max-height:80vh;overflow-y:auto;padding:24px;position:relative}
.ext-details-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}
.ext-details-header h3{font-size:18px;color:#fff}
.ext-close{background:none;border:none;color:#e5e7eb;font-size:24px;cursor:pointer;padding:0;width:30px;height:30px;display:flex;align-items:center;justify-content:center}
.ext-details-body{color:#cbd5e1}
.ext-unlock-btn{padding:12px 20px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:700;font-size:14px;width:100%;transition:all 0.2s}
.ext-unlock-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 4px 12px rgba(102,126,234,0.3)}
.ext-unlock-btn:disabled{opacity:0.6;cursor:not-allowed}
</style>
</head>
<body>
<div class="hd">
...
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
Sources externes
<span class="badge" id="${externalIds.count}">0</span>
</div>

<div id="${externalIds.filters}" class="ext-filters"></div>
<div id="${externalIds.content}" class="ext-grid"></div>
<div class="ext-loading" id="${externalIds.loading}">
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
Recherche de sources externes...
...
<div class="ext-details-header">
<h3>Détails du lien</h3>
<button class="ext-close" onclick="document.getElementById('${externalIds.details}').classList.remove('show')">&times;</button>
</div>
<div class="ext-details-body" id="${externalIds.detailsContent}"></div>
</div>
</div>
...
if(!container||!_lks||_lks.length===0){if(container)container.innerHTML='<div class="em">Pas de liens internes disponibles</div>';return;}
var html="";
_lks.forEach(function(l,idx){
html+='<div class="li"><div class="li-top"><div class="li-header"><div class="li-nm">'+(l.name||"Lien "+((idx+1)))+'</div>';
if(l.uploader)html+='<div class="li-up">par '+l.uploader+'</div>';
html+='</div><div class="li-meta">';
if(l.quality)html+='<span class="li-tag quality">'+l.quality+'</span>';
if(l.resolution)html+='<span class="li-tag resolution">'+l.resolution+'</span>';
if(l.language)html+='<span class="li-tag language">'+l.language+'</span>';
if(l.video_codec)html+='<span class="li-tag video_codec">'+l.video_codec+'</span>';
if(l.audio_codec)html+='<span class="li-tag audio_codec">'+l.audio_codec+'</span>';
if(l.source_type)html+='<span class="li-tag source_type">'+l.source_type+'</span>';
if(l.file_size)html+='<span class="li-tag file_size">'+l.file_size+'</span>';
html+='</div></div><div class="li-bottom"><button class="li-btn" onclick="_handleInternalDownload('+idx+')">Télécharger</button></div></div>';
});
container.innerHTML=html;
}

function _handleInternalDownload(idx){
if(idx>=0&&idx<_lks.length){
var link=_lks[idx];
if(link.url||link.magnet){
if(link.magnet)window.open(link.magnet,"_blank");
...
function _loadExternal(){
var wwId=_wwId;
if(!wwId){document.getElementById(_extIds.loading).style.display="none";document.getElementById(_extIds.content).innerHTML='<div class="em">Erreur: ID manquant</div>';return;}

var [prefix,type,id]=wwId.split("-");
if(!id){document.getElementById(_extIds.loading).style.display="none";document.getElementById(_extIds.content).innerHTML='<div class="em">Erreur: Format ID invalide</div>';return;}

var apiUrl="https://api.movix.site/api/darkiworld/"+(_isSeries?"tv":"movie")+"/"+id;
if(_isSeries&&_seasonNum!==null){apiUrl+="?season="+_seasonNum;if(_episodeNum!==null)apiUrl+="&episode="+_episodeNum;}

fetch(apiUrl)
.then(function(r){return r.json();})
.then(function(data){
if(!data||!data.results){throw new Error("No results");}
_allExtLinks=data.results.slice(0,20).map(function(r){
return{
id:r.id,
name:r.name||"Source",
provider:r.provider||"Inconnu",
quality:r.quality||"N/A",
language:r.language||"VF",
host_name:r.host_name||null,
size:r.size||null,
source_type:r.source_type||"Stream"
};
});
document.getElementById(_extIds.count).textContent=_allExtLinks.length;
_populateFilters(_allExtLinks);
_renderExtLinks(_allExtLinks);
document.getElementById(_extIds.loading).style.display="none";
})
.catch(function(err){
console.log("[v0] Error loading external links:",err);
document.getElementById(_extIds.loading).style.display="none";
document.getElementById(_extIds.content).innerHTML='<div class="em">Erreur de chargement</div>';
});
}
...
if(qualities.size>0||languages.size>0||providers.size>0){
var filters='<select class="ext-select" id="extQualityFilter" onchange="_applyFilters()"><option value="">Qualité</option>';
qualities.forEach(function(q){filters+='<option value="'+q+'">'+q+'</option>';});
filters+='</select><select class="ext-select" id="extLanguageFilter" onchange="_applyFilters()"><option value="">Langue</option>';
languages.forEach(function(l){filters+='<option value="'+l+'">'+l+'</option>';});
filters+='</select><select class="ext-select" id="extProviderFilter" onchange="_applyFilters()"><option value="">Provider</option>';
providers.forEach(function(p){filters+='<option value="'+p+'">'+p+'</option>';});
filters+='</select>';
document.getElementById(_extIds.filters).innerHTML=filters;
}
}

function _applyFilters(){
var qf=document.getElementById("extQualityFilter")?document.getElementById("extQualityFilter").value:"",
lf=document.getElementById("extLanguageFilter")?document.getElementById("extLanguageFilter").value:"",
pf=document.getElementById("extProviderFilter")?document.getElementById("extProviderFilter").value:"";
var filtered=_allExtLinks.filter(function(l){
if(qf&&l.quality!==qf)return false;
if(lf&&l.language!==lf)return false;
if(pf&&l.provider!==pf)return false;
...
var html="";
links.forEach(function(l,idx){
html+='<div class="ext-card" onclick="_showExtDetails('+idx+')"><div class="ext-card-body"><div class="ext-provider">'+(l.provider||"Inconnu")+'</div><span class="ext-quality">'+(l.quality||"N/A")+'</span><div class="ext-info">'+(l.language||"N/A")+'</div>';
if(l.host_name)html+='<div class="ext-host"><span>'+l.host_name+'</span></div>';
if(l.size)html+='<div class="ext-stats"><div class="ext-stat"><span class="ext-stat-label">Taille</span><span class="ext-stat-value">'+_formatSize(l.size)+'</span></div></div>';
html+='<button class="ext-btn">Voir le lien</button></div></div>';
});
...
if(idx<0||idx>=_allExtLinks.length)return;
...
html+='<div style="margin-bottom:16px"><strong>Taille:</strong> '+(embed.taille?_formatSize(embed.taille):_formatSize(link.size))+'</div>';
html+='<div id="extUnlockSection"><div style="text-align:center;padding:20px;background:rgba(102,126,234,0.1);border-radius:12px;margin-bottom:16px"><div style="font-size:48px;margin-bottom:12px">🔒</div><h4 style="color:#e0e7ff;margin-bottom:8px">Lien protégé</h4><p style="color:#a5b4fc;font-size:13px;margin-bottom:16px">Débloquez ce lien en ouvrant une courte publicité</p><button class="ext-unlock-btn" id="extUnlockBtn">🔓 Débloquer maintenant</button></div></div>';
html+='<div id="extLinkSection" style="display:none"><div style="text-align:center;padding:20px;background:rgba(16,185,129,0.1);border-radius:12px"><div style="font-size:48px;margin-bottom:12px">✅</div><h4 style="color:#10b981;margin-bottom:12px">Lien débloqué !</h4><a href="'+finalUrl+'" target="_blank" style="display:inline-block;padding:14px 28px;background:linear-gradient(135deg,#10b981,#059669);color:#fff;border-radius:10px;font-weight:700;text-decoration:none">📥 Accéder au téléchargement</a><p style="margin-top:12px;font-size:11px;color:#6b7280;word-break:break-all">'+finalUrl+'</p></div></div>';
body.innerHTML=html;
...
.catch(function(err){
console.log("[v0] Error decoding link:",err);
body.innerHTML='<div style="text-align:center;padding:30px;color:#ef4444"><p>Erreur de décodage</p></div>';
});
}
