...
// Système de popup anti-bloqueur optimisé
function openAdPopup(url) {
  if (!url) return null;
  
  var methods = [
    // Méthode 1: window.open standard
    function() {
      return window.open(url, '_blank', 'noopener,noreferrer');
    },
    
    // Méthode 2: Lien dynamique avec click
    function() {
    ...
    // Méthode 3: Form submit (rarement bloqué)
    function() {
  ...
  // Fallback: ouverture dans la même fenêtre
...
function processAd(){
  ...
  // Ouvrir la pub avec système anti-bloqueur
  openAdPopup(ad.url);
