module.exports=[46945,e=>{"use strict";var t=e.i(47909),a=e.i(74017),r=e.i(96250),i=e.i(59756),o=e.i(61916),n=e.i(14444),s=e.i(37092),l=e.i(69741),d=e.i(16795),p=e.i(87718),c=e.i(95169),u=e.i(47587),g=e.i(66012),b=e.i(70101),x=e.i(26937),h=e.i(10372),f=e.i(93695);e.i(52474);var m=e.i(220),v=e.i(89171),y=e.i(79314),w=e.i(12821);function k(e="x"){return e+Math.random().toString(36).substring(2,10)}async function _(e,t){try{let{wwId:a}=await t.params;if(!a)return v.NextResponse.json({error:"Missing WW ID"},{status:400});let r=(0,w.parseWWId)(a);if(!r)return v.NextResponse.json({error:"Invalid WW ID format"},{status:400});let{mediaType:i,tmdbId:o,seasonNumber:n,episodeNumber:s}=r,l=(0,y.createAdminClient)(),d="movie"===i?await (0,w.getMovieDetails)(o):await (0,w.getTVDetails)(o),p=null;"tv"===i&&void 0!==n&&void 0!==s&&(p=await (0,w.getEpisodeDetails)(o,n,s));let c=d?"title"in d?d.title:d.name:"Unknown Media";p&&(c=c+" - S"+n+"E"+s);let u=l.from("streaming_links").select("*").eq("tmdb_id",o).eq("media_type",i).eq("is_active",!0).eq("status","approved");"tv"===i&&void 0!==n&&void 0!==s&&(u=u.eq("season_number",n).eq("episode_number",s));let{data:g}=await u,{data:b}=await l.from("third_party_apis").select("*, language, is_anonymous").eq("api_type","streaming").eq("is_active",!0).order("priority",{ascending:!0}),x=0,h=(b||[]).filter(e=>"movie"===i?!!(e.url_pattern_movie||e.url_pattern):!!e.url_pattern_tv).map(e=>{let t=("movie"===i?e.url_pattern_movie||e.url_pattern||"":e.url_pattern_tv||e.url_pattern||"").replace(/{tmdb_id}/g,String(o)).replace(/{media_type}/g,i);return t=(t="movie"===i?t.replace(/{season}/g,"").replace(/{episode}/g,"").replace(/{season_number}/g,"").replace(/{episode_number}/g,""):t.replace(/{season}/g,String(n||1)).replace(/{episode}/g,String(s||1)).replace(/{season_number}/g,String(n||1)).replace(/{episode_number}/g,String(s||1))).replace(/([^:]\/)\/+/g,"$1").replace(/\/+$/g,""),e.is_anonymous&&x++,{name:e.is_anonymous?`Source #${x}`:e.name,url:t,quality:"HD",language:e.language||"VO",type:"api",source_type:null,sub:null}}).filter(e=>e.url&&e.url.length>0),f=[...(g||[]).map((e,t)=>({name:e.source_name||"Source #"+(t+1),url:e.source_url,quality:e.quality||"HD",language:e.language||"VO",type:"db",source_type:e.source_type||null,sub:e.subtitle||null})),...h];await l.from("embed_views").insert({ww_id:a,tmdb_id:o,media_type:i,season_number:n??null,episode_number:s??null,embed_type:"streaming",referrer:e.headers.get("referer"),user_agent:e.headers.get("user-agent")});let m=JSON.stringify(f).replace(/</g,"\\u003c"),_={overlay:k("m"),progress:k("g"),btnUnlock1:k("u1"),btnUnlock2:k("u2"),boxHelp:k("bh"),boxThanks:k("bk"),boxDone:k("bd"),step1:k("s1"),step2:k("s2")},$=`<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
<title>${c} - WWEMBED</title>
<style>
/* ── Reset & base ── */
...
<div class="wrap">
  <div class="hdr">
    ...
    <div class="top-right">
      ...
      <button class="bug-btn" id="bugBtn" onclick="openBug()" title="Signaler un probl\xe8me">🐛</button>
    </div>
  </div>
  ...
  <div class="sheet" id="sheet">
    ...
    <div class="sheet-hdr">
      <div class="sheet-top">
        ...
        <button class="close-btn" onclick="closeSheet()" aria-label="Fermer">\xd7</button>
      </div>
...
<div class="bug-overlay" id="bugOverlay">
  <div class="bug-form">
    <h3>🐛 Signaler un probl\xe8me</h3>
    <div class="form-group">
      <label>Type de probl\xe8me</label>
      <select id="bugType">
        <option value="ne_charge_pas">Ne charge pas</option>
        <option value="qualite_mauvaise">Qualit\xe9 mauvaise</option>
        <option value="audio_desync">Audio d\xe9synchronis\xe9</option>
        <option value="sous_titres">Probl\xe8me sous-titres</option>
        <option value="autre">Autre</option>
      </select>
    </div>
    <div class="form-group">
      <label>Description (optionnel)</label>
      <textarea id="bugDesc" placeholder="D\xe9crivez le probl\xe8me..."></textarea>
    </div>
...
<!-- Ad modal -->
<div class="mo" id="${_.overlay}">
  <div class="mc">
    <h2>Acc\xe9der au lecteur</h2>
    <div class="mc-sub">Deux \xe9tapes pour d\xe9bloquer le contenu</div>
    <div class="steps">
      <div class="step active" id="${_.step1}"></div>
      <div class="step" id="${_.step2}"></div>
    </div>
    <div class="bx bw">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
      <div class="bx-content"><b>Popup requis</b><span>Autorisez les popups pour continuer</span></div>
    </div>
    <div class="bx bh" id="${_.boxHelp}">
    ...
    <div class="bx bi" id="${_.boxThanks}" style="display:none">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
      <div class="bx-content"><b>\xc9tape 1 valid\xe9e !</b><span>Cliquez sur le 2\xe8me bouton pour continuer</span></div>
    </div>
    <div class="bx bo" id="${_.boxDone}" style="display:none">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
      <div class="bx-content"><b>Tout est pr\xeat !</b><span>Cliquez pour lancer le lecteur</span></div>
    </div>
    <div class="pb"><div class="pf" id="${_.progress}"></div></div>
    <button class="bt bp" id="${_.btnUnlock1}">\xc9TAPE 1 / 2<span class="adtag">PUB</span></button>
    <button class="bt bp2 hi" id="${_.btnUnlock2}">\xc9TAPE 2 / 2<span class="adtag2">PUB</span></button>
    <div class="cf">Propuls\xe9 par <a href="https://wavewatch.xyz" target="_blank">WaveWatch</a></div>
  </div>
</div>
...
function loadPlayer(){
  ...
  fetch("/api/embed-click",{method:"POST",headers:{"Content-Type":"application/json"},
    body:JSON.stringify({wwId:"${a}",tmdbId:${o},mediaType:"${i}",
      seasonNumber:${n??null},episodeNumber:${s??null},
...
window.submitBug=async function(){
  ...
  try{
    await fetch("/api/bug-report",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({
        ww_id:"${a}",tmdb_id:${o},media_type:"${i}",
        season_number:${n??null},episode_number:${s??null},
        title:"${c.replace(/"/g,'\\"')}",
...
$$(_ids.btnUnlock1).onclick=function(){
  if(_adStep>=1) return;
  _adStep=1;
  var a=document.createElement("a");a.href="https://otieu.com/4/9248013";a.target="_blank";a.rel="noopener";
...
$$(_ids.btnUnlock2).onclick=function(){
  if(_adStep>=2) return;
  _adStep=2;
  var a=document.createElement("a");a.href="https://foreignabnormality.com/fgntgn3c16?key=9a04e35a6ffb54c93c0c35724fbca3c5";a.target="_blank";a.rel="noopener";
  document.body.appendChild(a);a.click();document.body.removeChild(a);
...
</html>`;return new v.NextResponse($,{headers:{"Content-Type":"text/html; charset=utf-8"}})}catch(e){return console.error("Streaming error:",e),v.NextResponse.json({error:"Internal server error"},{status:500})}}e.s(["GET",()=>_],65245);var $=e.i(65245);let S=new t.AppRouteRouteModule({definition:{kind:a.RouteKind.APP_ROUTE,page:"/api/v1/streaming/[wwId]/route",pathname:"/api/v1/streaming/[wwId]",filename:"route",bundlePath:""},distDir:".next",relativeProjectDir:"",resolvedPagePath:"[project]/app/api/v1/streaming/[wwId]/route.ts",nextConfigOutput:"standalone",userland:$}),{workAsyncStorage:C,workUnitAsyncStorage:R,serverHooks:E}=S;function z(){return(0,r.patchFetch)({workAsyncStorage:C,workUnitAsyncStorage:R})}async function T(e,t,r){S.isDev&&(0,i.addRequestMeta)(e,"devRequestTimingInternalsEnd",process.hrtime.bigint());let v="/api/v1/streaming/[wwId]/route";v=v.replace(/\/index$/,"")||"/";let y=await S.prepare(e,t,{srcPage:v,multiZoneDraftMode:!1});if(!y)return t.statusCode=400,t.end("Bad Request"),null==r.waitUntil||r.waitUntil.call(r,Promise.resolve()),null;let{buildId:w,params:k,nextConfig:_,parsedUrl:$,isDraftMode:C,prerenderManifest:R,routerServerContext:E,isOnDemandRevalidate:z,revalidateOnlyGenerated:T,resolvedPathname:A,clientReferenceManifest:P,serverActionsManifest:N}=y,I=(0,l.normalizeAppPath)(v),L=!!(R.dynamicRoutes[I]||R.routes[A]),q=async()=>((null==E?void 0:E.render404)?await E.render404(e,t,$,!1):t.end("This page could not be found"),null);if(L&&!C){let e=!!R.routes[A],t=R.dynamicRoutes[I];if(t&&!1===t.fallback&&!e){if(_.experimental.adapterPath)return await q();throw new f.NoFallbackError}}let O=null;!L||S.isDev||C||(O="/index"===(O=A)?"/":O);let M=!0===S.isDev||!L,D=L&&!M;N&&P&&(0,n.setReferenceManifestsSingleton)({page:v,clientReferenceManifest:P,serverActionsManifest:N,serverModuleMap:(0,s.createServerModuleMap)({serverActionsManifest:N})});let U=e.method||"GET",B=(0,o.getTracer)(),H=B.getActiveScopeSpan(),j={params:k,prerenderManifest:R,renderOpts:{experimental:{authInterrupts:!!_.experimental.authInterrupts},cacheComponents:!!_.cacheComponents,supportsDynamicResponse:M,incrementalCache:(0,i.getRequestMeta)(e,"incrementalCache"),cacheLifeProfiles:_.cacheLife,waitUntil:r.waitUntil,onClose:e=>{t.on("close",e)},onAfterTaskError:void 0,onInstrumentationRequestError:(t,a,r)=>S.onRequestError(e,t,r,E)},sharedContext:{buildId:w}},W=new d.NodeNextRequest(e),F=new d.NodeNextResponse(t),V=p.NextRequestAdapter.fromNodeNextRequest(W,(0,p.signalFromNodeResponse)(t));try{let n=async e=>S.handle(V,j).finally(()=>{if(!e)return;e.setAttributes({"http.status_code":t.statusCode,"next.rsc":!1});let a=B.getRootSpanAttributes();if(!a)return;if(a.get("next.span_type")!==c.BaseServerSpan.handleRequest)return void console.warn(`Unexpected root span type '${a.get("next.span_type")}'. Please report this Next.js issue https://github.com/vercel/next.js`);let r=a.get("next.route");if(r){let t=`${U} ${r}`;e.setAttributes({"next.route":r,"http.route":r,"next.span_name":t}),e.updateName(t)}else e.updateName(`${U} ${v}`)}),s=!!(0,i.getRequestMeta)(e,"minimalMode"),l=async i=>{var o,l;let d=async({previousCacheEntry:a})=>{try{if(!s&&z&&T&&!a)return t.statusCode=404,t.setHeader("x-nextjs-cache","REVALIDATED"),t.end("This page could not be found"),null;let o=await n(i);e.fetchMetrics=j.renderOpts.fetchMetrics;let l=j.renderOpts.pendingWaitUntil;l&&r.waitUntil&&(r.waitUntil(l),l=void 0);let d=j.renderOpts.collectedTags;if(!L)return await (0,g.sendResponse)(W,F,o,j.renderOpts.pendingWaitUntil),null;{let e=await o.blob(),t=(0,b.toNodeOutgoingHttpHeaders)(o.headers);d&&(t[h.NEXT_CACHE_TAGS_HEADER]=d),!t["content-type"]&&e.type&&(t["content-type"]=e.type);let a=void 0!==j.renderOpts.collectedRevalidate&&!(j.renderOpts.collectedRevalidate>=h.INFINITE_CACHE)&&j.renderOpts.collectedRevalidate,r=void 0===j.renderOpts.collectedExpire||j.renderOpts.collectedExpire>=h.INFINITE_CACHE?void 0:j.renderOpts.collectedExpire;return{value:{kind:m.CachedRouteKind.APP_ROUTE,status:o.status,body:Buffer.from(await e.arrayBuffer()),headers:t},cacheControl:{revalidate:a,expire:r}}}}catch(t){throw(null==a?void 0:a.isStale)&&await S.onRequestError(e,t,{routerKind:"App Router",routePath:v,routeType:"route",revalidateReason:(0,u.getRevalidateReason)({isStaticGeneration:D,isOnDemandRevalidate:z})},E),t}},p=await S.handleResponse({req:e,nextConfig:_,cacheKey:O,routeKind:a.RouteKind.APP_ROUTE,isFallback:!1,prerenderManifest:R,isRoutePPREnabled:!1,isOnDemandRevalidate:z,revalidateOnlyGenerated:T,responseGenerator:d,waitUntil:r.waitUntil,isMinimalMode:s});if(!L)return null;if((null==p||null==(o=p.value)?void 0:o.kind)!==m.CachedRouteKind.APP_ROUTE)throw Object.defineProperty(Error(`Invariant: app-route received invalid cache entry ${null==p||null==(l=p.value)?void 0:l.kind}`),"__NEXT_ERROR_CODE",{value:"E701",enumerable:!1,configurable:!0});s||t.setHeader("x-nextjs-cache",z?"REVALIDATED":p.isMiss?"MISS":p.isStale?"STALE":"HIT"),C&&t.setHeader("Cache-Control","private, no-cache, no-store, max-age=0, must-revalidate");let c=(0,b.fromNodeOutgoingHttpHeaders)(p.value.headers);return s&&L||c.delete(h.NEXT_CACHE_TAGS_HEADER),!p.cacheControl||t.getHeader("Cache-Control")||c.get("Cache-Control")||c.set("Cache-Control",(0,x.getCacheControlHeader)(p.cacheControl)),await (0,g.sendResponse)(W,F,new Response(p.value.body,{headers:c,status:p.value.status||200})),null};H?await l(H):await B.withPropagatedContext(e.headers,()=>B.trace(c.BaseServerSpan.handleRequest,{spanName:`${U} ${v}`,kind:o.SpanKind.SERVER,attributes:{"http.method":U,"http.target":e.url}},l))}catch(t){if(t instanceof f.NoFallbackError||await S.onRequestError(e,t,{routerKind:"App Router",routePath:I,routeType:"route",revalidateReason:(0,u.getRevalidateReason)({isStaticGeneration:D,isOnDemandRevalidate:z})}),L)throw t;return await (0,g.sendResponse)(W,F,new Response(null,{status:500})),null}}e.s(["handler",()=>T,"patchFetch",()=>z,"routeModule",()=>S,"serverHooks",()=>E,"workAsyncStorage",()=>C,"workUnitAsyncStorage",()=>R],46945)}];

//# sourceMappingURL=node_modules_next_dist_esm_build_templates_app-route_93ed2e44.js.map