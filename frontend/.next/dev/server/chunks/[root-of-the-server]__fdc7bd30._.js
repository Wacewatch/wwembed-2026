...
"[project]/app/api/check-link/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
    ...
    try {
        ...
        try {
            ...
            if (response.status === 403 || response.status === 405) {
                ...
                return {
                    isValid: getResponse.status >= 200 && getResponse.status < 300
                };
            }
            return {
                isValid: response.ok
...
async function GET(request) {
    ...
    const { data: downloadLinks } = await supabase.from("download_links").select("id, source_url").or(`last_checked.is.null,last_checked.lt.${twelveHoursAgo.toISOString()}`).eq("is_active", true).limit(50);
    const { data: digitalLinks } = await supabase.from("digital_download_links").select("id, source_url").or(`last_checked.is.null,last_checked.lt.${twelveHoursAgo.toISOString()}`).eq("is_active", true).limit(50);
    const { data: streamingLinks } = await supabase.from("streaming_links").select("id, source_url").or(`last_checked.is.null,last_checked.lt.${twelveHoursAgo.toISOString()}`).eq("is_active", true).limit(50);
    const results = {
        checked: 0,
        valid: 0,
        invalid: 0
...
//# sourceMappingURL=%5Broot-of-the-server%5D__fdc7bd30._.js.map