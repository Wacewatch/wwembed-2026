...
"[project]/app/api/v1/links/[wwId]/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request, { params }) {
    ...
    // Get streaming links
    const { data: streamingLinks } = await supabase.from("streaming_links").select("id, source_name, source_url, quality, language, is_verified").eq("tmdb_id", tmdbId).eq("media_type", mediaType).eq("is_active", true);
    ...
    // Get download links
    const { data: downloadLinks } = await supabase.from("download_links").select("id, source_name, source_url, link_type, quality, file_size, language, is_verified").eq("tmdb_id", tmdbId).eq("media_type", mediaType).eq("is_active", true);
    // Log API usage
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f2bae1fd._.js.map